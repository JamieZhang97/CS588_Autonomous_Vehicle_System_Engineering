...
class PurePursuit(object):
    ...
    def read_waypoints_from_lidar_clusters(self):
        ...
        avg_centroids, way_points = get_waypoints_main(as_main=False)
        ...
        self.path_points_lon_x   = xs_merged # longitude
        self.path_points_lat_y   = ys_merged # latitude
        self.path_points_heading = heading # heading
        print("way_points x:", xs_merged[:10])
        print("way_points y:", ys_merged[:10])
        print("way_points heading:", heading[:10])
    ...
    def read_waypoints(self):
        ...
        if MP_NAME == "mp3":
            filename = os.path.join(dirname, '../waypoints/mp4_demo.csv')
        else:
        ...
        with open(filename) as f:
            ...
            for i,line in enumerate(csv.reader(f)):
                if i != 0:
                    path_points.append(tuple(line))
                    print(tuple(line))
            # quit()           
        # x towards East and y towards North
        if MP_NAME == "mp3":
            self.path_points_lon_x   = [float(point[1]) for point in path_points] # longitude
            self.path_points_lat_y   = [float(point[2]) for point in path_points] # latitude
        else:
            self.path_points_lon_x   = [-float(point[1]) for point in path_points] # longitude
            self.path_points_lat_y   = [-float(point[2]) for point in path_points] # latitude
            
        self.path_points_heading = [float(point[3]) for point in path_points] # heading
        self.wp_size             = len(self.path_points_lon_x)
        self.dist_arr            = np.zeros(self.wp_size)

        self.stop_idx_ratio = 0.9
